...
#
#   Correlated feature pairs (Nc–NRc r=0.96, T50–phi r=−0.82)
#   hurt Random Forest's random feature subsampling: when two
#   correlated features are both in the candidate pool, one
#   split selection is effectively wasted. PCA produces
#   orthogonal components so every candidate at a split carries
#   independent information. Fit on training data only.
# ----------------------------------------------------------
pca = PCA(n_components=0.95, random_state=42)
X_tr_p = pca.fit_transform(X_tr_s)
X_va_p = pca.transform(X_va_s)
X_te_p = pca.transform(X_te_s)
print(f"PCA: retained {pca.n_components_} components (≥95% variance)\n")
...
#
#   RF importances are per PCA component. We back-project to
#   the original feature space for interpretability:
#     importance_j = Σ_k |loading_kj| × component_importance_k
# ----------------------------------------------------------
...
orig_imp.plot.barh(ax=ax, color='steelblue')
ax.set_xlabel('Back-projected importance (via PCA loadings)')
...
#     inside CV so scaler and PCA are fit only on training folds —
#     no information leaks from the held-out validation fold.
#
...
WINDOW_CANDIDATES = [10, 20, 30, 50, 75]
